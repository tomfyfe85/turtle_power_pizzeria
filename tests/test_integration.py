...
"""
Given a multiple orders in the basket, Order#view_basket will return a formatted receipt of the item and the grand total
"""


def test_view_basket_returns_a_formatted_receipt_and_grand_total_of_multiple_items():
    menu = Menu()
    order = Order(menu)

    order.add_to_basket(2, 2)
    order.add_to_basket(3, 1)
    assert (
        order.view_basket()
        == "Your order:\n\n American Hot - £11.99\n American Hot - £11.99\n Margarita - £8.99\n\n Grand total: £32.97\n\n Thank you!"
    )
